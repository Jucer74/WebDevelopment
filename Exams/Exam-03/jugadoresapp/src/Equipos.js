...
function Equipos() {
  ...
  const handleSearch =  async(e) => {
    e.preventDefault();
    return await axios.get(`http://localhost:5000/equipos?q=${value}`).then((response)=>{
      setData(response.data); 
      setValue("");
  ...
  return (
    ...
    <Row>
      <Col md={2}>
        ...
        <input type="text" className="form-control" placeholder="Buscar por liga" value={value} onChange={(e)=>setValue(e.target.value)}/>
        <ButtonGroup>
    ...
    <Container style={{marginTop:"10px"}}>
      <Row>
        ...
        <Form onSubmit={handleSubmit}>
          ...
          <Form.Control type="text" placeholder="Liga" name="Liga"  value={Liga} onChange={handleChange}/>
          </Form.Group>
